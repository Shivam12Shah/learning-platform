...
const Coursedetails = () => {
  ...
  const { id } = useParams();
  console.log(user.coursebuy.includes(id));
  ...
  const handlePayment = async () => {
    if(user.coursebuy.includes(id)){
      alert("You Already buy the course")
      return;
    }
    createOrderId();
  ...
  return (
    <div className="coursedets w-full flex p-20 gap-10 max-sm:w-full max-sm:flex-col max-sm:p-5">
      <div className="coursemaindeetials w-[60%]">
        ...
        <p className="text-sm bg-[#CDC2FF] mt-2 w-max px-2 py-1 rounded-md">
          Course Name
        </p>
      ...
      <div className="imagediv-and-buy-option w-[40%] max-sm:w-full">
        ...
        <button
          ...
          className="bg-[#8B77E8] text-white px-2 py-1 mt-2 w-full capitalize rounded-md"
        >
          Buy Now
        </button>
        <button className="bg-[#8B77E8] text-white px-2 py-1 mt-2 w-full capitalize rounded-md">
          ${newprice}
        </button>
      </div>
    </div>
